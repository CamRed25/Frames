...
//! between `parapet_core` data providers and `parapet_bar` renderers.
//!
//! Every data-producing component implements [`Widget`]. Renderers in
//! `parapet_bar` consume [`WidgetData`] values returned by `Widget::update`.
...
pub trait Widget: Send + Sync {
    ...
    /// # Errors
    ///
    /// Returns [`ParapetError`] only for persistent, unrecoverable failures.
